...
int do_init(void) {
    ...
    if (XIic_CfgInitialize(&IicInstance, ConfigPtr, ConfigPtr->BaseAddress) != XST_SUCCESS) return XST_FAILURE;
...
float pid_funct(PID_t* pid, uint16_t lux_value, uint8_t switches) {
    ...
    pid->integral += error;
    if (pid->integral > 2048) pid->integral = 2048;
    else if (pid->integral < -2048) pid->integral = -2048;
...
void PID_Task(void* p) {
    ...
    while (1) {
        ...
        else pwmLED += (uint8_t)(pidOUT * max_duty);
...
int main(void) {
    prvSetupHardware();
    if (do_init() != XST_SUCCESS) while (1);
    if (tsl2561_init(&IicInstance) != XST_SUCCESS) while (1);
    if (bme_init(&IicInstance) != 0) while (1);
    if (bme_read_calibration_data(&IicInstance, &calib_data) != 0) while (1);

    oled.iic = &IicInstance;
    oled.sem = xSemaphoreCreateBinary();
    if (oled.sem == NULL) while (1);
    xSemaphoreGive(oled.sem);
    oled_init(&oled);

    vSemaphoreCreateBinary(binary_sem);
    if (binary_sem == NULL) while (1);

    bme280_sem = xSemaphoreCreateBinary();
    if (bme280_sem == NULL) while (1);
    xSemaphoreGive(bme280_sem);

    toPID = xQueueCreate(mainQUEUE_LENGTH, sizeof(uint16_t));
    fromPID = xQueueCreate(mainQUEUE_LENGTH, sizeof(uint32_t));
    if (!toPID || !fromPID) while (1);
